...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    return Response.json({ error: "恢复请求不符合合同" }, { status: 400 });
  }

  const decodedRequest = decodeRuntimeResumeRequestV1(requestBody);
  ...
  try {
    const response = await fetch(
      `${apiUrl}/api/v1/decision-tasks/${encodeURIComponent(decisionTaskId)}/resume`,
      {
        method: "POST",
        cache: "no-store",
        headers,
    ...
    if (
      ...
      decodedStatus.value.action === "RESUME" &&
      decodedStatus.value.controlRequestId === decodedRequest.value.controlRequestId
    ) {
      return Response.json(decodedStatus.value, { status: 202 });
    }
  } catch {
    // 统一在下方失败关闭，避免向浏览器泄露上游连接细节。
  }

  return Response.json({ error: "恢复请求状态暂时无法确认" }, { status: 503 });
}
