...
if ($actualNodeVersion -ne $requiredNodeVersion) {
    ...
    if ($null -eq $actualNodeVersion) {
        [Console]::Error.WriteLine("错误：缺少必需命令：node，也未找到可自动切换到 $requiredNodeVersion 的 fnm。请安装 Node.js $requiredNodeVersion 或 fnm 后重试。")
    }
    else {
        [Console]::Error.WriteLine("错误：Node.js 版本不匹配：需要 $requiredNodeVersion，当前为 $actualNodeVersion，且无法通过 fnm 自动切换。请安装 fnm，或手动切换到 Node.js $requiredNodeVersion 后重试。")
    }
    exit 1
}
